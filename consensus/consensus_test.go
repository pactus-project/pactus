...
func (td *testData) commitBlockForAllStates(t *testing.T) (*block.Block, *certificate.Certificate) {
	...
	sb := certificate.BlockCertificateSignBytesFastPath(prop.Block().Hash(), height+1, 0)
	...
	cert := certificate.NewCertificateFastPath(height+1, 0, []int32{0, 1, 2, 3, 4, 5}, []int32{5}, sig)
	blk := prop.Block()
...
// 	td.addCPMainVote(td.consP, blockHash, h, r, 0, vote.CPValueZero, mainVote0.CPJust(), tIndexX)
// 	td.addCPMainVote(td.consP, blockHash, h, r, 0, vote.CPValueZero, mainVote0.CPJust(), tIndexY)
...
func TestPickRandomVote(t *testing.T) {
	...
	cpPreVote := certificate.BlockCertificateSignBytes(hash.UndefHash, 1, 0)
	cpPreVote = append(cpPreVote, util.StringToBytes(vote.VoteTypeCPPreVote.String())...)
	cpPreVote = append(cpPreVote, util.Int16ToSlice(cpRound)...)
	cpPreVote = append(cpPreVote, byte(vote.CPValueYes))

	cpMainVote := certificate.BlockCertificateSignBytes(hash.UndefHash, 1, 0)
	cpMainVote = append(cpMainVote, util.StringToBytes(vote.VoteTypeCPMainVote.String())...)
	cpMainVote = append(cpMainVote, util.Int16ToSlice(cpRound)...)
	cpMainVote = append(cpMainVote, byte(vote.CPValueYes))
	...
	for i, val := range td.consP.validators {
		...
		preVoteSigs = append(preVoteSigs, td.valKeys[i].Sign(cpPreVote))
		mainVoteSigs = append(mainVoteSigs, td.valKeys[i].Sign(cpMainVote))
	}
...
func TestByzantine(t *testing.T) {
	...
	byzJust0Block := &vote.JustInitZero{
		QCert: td.consB.makeCertificate(
