...
func TestMessageCompress(t *testing.T) {
	...
	pld := message.NewBlocksResponseMessage(message.ResponseCodeBusy, 1234, 888, blocks, trxs, nil)
	msg := NewBundle(util.RandomPeerID(), pld)
	bs0, err := msg.Encode()
	...
	assert.True(t, util.IsFlagSet(msg.Flags, BundleFlagCompressed))
}
