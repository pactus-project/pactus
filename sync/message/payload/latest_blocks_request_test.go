...
func TestLatestBlocksRequestPayload(t *testing.T) {
	t.Run("Invalid target", func(t *testing.T) {
		p1 := NewLatestBlocksRequestPayload(1, "", 100, 200)
		assert.Error(t, p1.SanityCheck())
	})

	t.Run("Invalid from", func(t *testing.T) {
		p1 := NewLatestBlocksRequestPayload(1, util.RandomPeerID(), -100, 200)
		assert.Error(t, p1.SanityCheck())
	})

	t.Run("Invalid range", func(t *testing.T) {
		p1 := NewLatestBlocksRequestPayload(1, util.RandomPeerID(), 200, 100)
		assert.Error(t, p1.SanityCheck())
	})
