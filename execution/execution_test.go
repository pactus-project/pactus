...
func TestTransferLockTime(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "Transaction has valid LockTime in far future",
			lockTime:     1014,
...
func TestExecute(t *testing.T) {
	...
	mockExe := executor.NewMockExecutor(ts.MockController())
	executor.DefaultFactory = func(*tx.Tx, sandbox.Sandbox) (executor.Executor, error) {
		return mockExe, nil
	}
	...
	t.Run("Valid transaction", func(t *testing.T) {
		...
		mockExe.EXPECT().Execute(gomock.Any()).Return().Times(1)
...
func TestCheck(t *testing.T) {
	...
	t.Run("Invalid lock-time", func(t *testing.T) {
		...
		sbx.EXPECT().CurrentHeight().Return(lockTime - 1).Times(2)
	...
	t.Run("Check fails", func(t *testing.T) {
		...
		sbx.EXPECT().RecentTransaction(trx.ID()).Return(false).Times(1)
		sbx.EXPECT().Params().Return(&param.Params{}).Times(1)
		sbx.EXPECT().CurrentHeight().Return(lockTime).Times(2)
		mockExe.EXPECT().Check(gomock.Any(), true).Return(errors.New("Some error")).Times(1)
	...
	t.Run("Ok", func(t *testing.T) {
		...
		sbx.EXPECT().Params().Return(&param.Params{}).Times(1)
		sbx.EXPECT().CurrentHeight().Return(lockTime).Times(2)
		mockExe.EXPECT().Check(gomock.Any(), true).Return(nil).Times(1)
		mockExe.EXPECT().Execute(gomock.Any()).Return().Times(1)
		sbx.EXPECT().CommitTransaction(trx).Return().Times(1)
