...
func TestExecuteWithdrawTx(t *testing.T) {
	...
	t.Run("Should fail, Invalid validator", func(t *testing.T) {
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), 1, crypto.GenerateTestAddress(), addr, 4999999000, 1000, "invalid validator")
		assert.Error(t, exe.Execute(trx, tSandbox))
	})

	t.Run("Should fail, Invalid sequence", func(t *testing.T) {
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), tVal1.Sequence()+2, tVal1.Address(), addr, 4999999000, 1000, "invalid sequence")
		assert.Error(t, exe.Execute(trx, tSandbox))
	})

	t.Run("Should fail, insufficient balance", func(t *testing.T) {
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), tVal1.Sequence()+1, tVal1.Address(), addr, tVal1.Stake()+1, 0, "insufficient balance")
	...
	t.Run("Should fail, hasn't unbonded yet", func(t *testing.T) {
		...
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), tVal1.Sequence()+1, tVal1.Address(), addr, 4999999000, 1000, "need to unbond first")
		assert.Error(t, exe.Execute(trx, tSandbox))
	})
	...
	t.Run("Should fail, hasn't passed unbonding interval", func(t *testing.T) {
		...
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), tVal1.Sequence()+1, tVal1.Address(), addr, 4999999000, 1000, "not passed unbonding interval")
		assert.Error(t, exe.Execute(trx, tSandbox))
	})
	...
	t.Run("Should pass, Everything is Ok!", func(t *testing.T) {
		total := tVal1.Stake()
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), tVal1.Sequence()+1, tVal1.Address(), addr, total-1000, 1000, "should be able to empty stake")
	...
	t.Run("Should fail, can't withdraw empty stake", func(t *testing.T) {
		...
		trx := tx.NewWithdrawTx(tHash500000.Stamp(), tVal1.Sequence()+1, tVal1.Address(), addr, 4999999000, 1000, "can't withdraw empty stake")
		assert.Error(t, exe.Execute(trx, tSandbox))
	})
