...
func TestExecuteUnbondTx(t *testing.T) {
	...
	t.Run("Should fail, Invalid validator", func(t *testing.T) {
		trx := tx.NewUnbondTx(tHash1000000.Stamp(), 1, addr, "invalid validator")
		assert.Error(t, exe.Execute(trx, tSandbox))
	})

	t.Run("Should fail, Invalid sequence", func(t *testing.T) {
		trx := tx.NewUnbondTx(tHash1000000.Stamp(), tSandbox.Validator(tVal1.Address()).Sequence()+2, tVal1.Address(), "invalid sequence")
	...
	t.Run("Should fail, Inside committee", func(t *testing.T) {
		...
		trx := tx.NewUnbondTx(tHash1000000.Stamp(), tSandbox.Validator(tVal1.Address()).Sequence()+1, tVal1.Address(), "inside committee")
	...
	t.Run("Ok", func(t *testing.T) {
		...
		trx := tx.NewUnbondTx(tHash1000000.Stamp(), tSandbox.Validator(tVal1.Address()).Sequence()+1, tVal1.Address(), "Ok")
...
func TestUnbondNonStrictMode(t *testing.T) {
	...
	trx := tx.NewUnbondTx(tHash1000000.Stamp(), tSandbox.ValSeq(tVal1.Address())+1, tVal1.Address(), "")
