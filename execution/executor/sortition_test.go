...
func TestExecuteSortitionTx(t *testing.T) {
	...
	proof1 := tVal1Signer.Sign(stamp40.RawBytes()).RawBytes()
	trx1 := tx.NewSortitionTx(stamp40, 1, addr1, proof1, "invalid-address", &pub1, nil)
	trx1.SetSignature(priv1.Sign(trx1.SignBytes()))
	assert.Error(t, exe.Execute(trx1))

	trx2 := tx.NewSortitionTx(stamp40, 2, tVal1Signer.Address(), proof1, "invalid-sequence", &tVal1Pub, nil)
	tVal1Signer.SignMsg(trx2)
	assert.Error(t, exe.Execute(trx2))

	trx3 := tx.NewSortitionTx(stamp40, 1, tVal1Signer.Address(), proof1, "too early", &tVal1Pub, nil)
	tVal1Signer.SignMsg(trx3)
	assert.Error(t, exe.Execute(trx3))

	tSandbox.AppendStampAndUpdateHeight(41, stamp41)

	trx4 := tx.NewSortitionTx(stamp41, 1, tVal1Signer.Address(), proof1, "ok", &tVal1Pub, nil)
