...
func TestExecuteSortitionTx(t *testing.T) {
	...
	val.UpdateLastBondingHeight(lockTime.SafeDecrease(td.sbx.Params().BondInterval + 1))
	...
	t.Run("Should fail, Bonding period", func(t *testing.T) {
		val.UpdateLastBondingHeight(lockTime - 1)
	...
	t.Run("Should fail, committee has free seats and validator is in the committee", func(t *testing.T) {
		...
		td.committee.EXPECT().Size().Return(td.params.CommitteeSize - 1).Times(1)
	...
	t.Run("Should be ok", func(t *testing.T) {
		...
		td.committee.EXPECT().Size().Return(td.params.CommitteeSize - 1).Times(1)
		td.sbx.EXPECT().JoinedToCommittee(val.Address()).Return().Times(1)
