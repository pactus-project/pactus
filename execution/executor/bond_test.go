...
func TestExecuteBondTx(t *testing.T) {
	...
	trx1 := tx.NewBondTx(stamp, 1, addr1, pub1, 1000, 1000, "invalid boner", &pub1, nil)
	tAcc1Signer.SignMsg(trx1)
	assert.Error(t, exe.Execute(trx1))

	trx2 := tx.NewBondTx(stamp, tSandbox.AccSeq(bonder)+2, bonder, pub1, 1000, 1000, "invalid sequence", &tAcc1Pub, nil)
	signer1.SignMsg(trx2)
	assert.Error(t, exe.Execute(trx2))

	trx3 := tx.NewBondTx(stamp, tSandbox.AccSeq(bonder)+1, bonder, pub1, 10000000000, 10000000, "insufficient balance", &tAcc1Pub, nil)
	signer1.SignMsg(trx3)
	assert.Error(t, exe.Execute(trx3))

	trx5 := tx.NewBondTx(stamp, tSandbox.AccSeq(bonder)+1, bonder, pub1, 1000, 1000, "ok", &tAcc1Pub, nil)
