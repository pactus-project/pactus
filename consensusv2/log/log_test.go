...
func TestAddValidVote(t *testing.T) {
	...
	vote1 := vote.NewPrecommitVote(ts.RandHash(), height, round, val.Address())
	vote2 := vote.NewCPPreVote(ts.RandHash(), height, round, 0, vote.CPValueYes, &vote.JustInitYes{}, val.Address())
	vote3 := vote.NewCPMainVote(ts.RandHash(), height, round, 0, vote.CPValueYes, &vote.JustInitYes{}, val.Address())
	vote4 := vote.NewCPDecidedVote(ts.RandHash(), height, round, 0, vote.CPValueNo, &vote.JustInitYes{}, val.Address())
...
func TestTotalPower(t *testing.T) {
	...
	log.MoveToNewHeight([]*validator.Validator{val1, val2})
	assert.Equal(t, val1.Power()+val2.Power(), log.TotalPower())
}
