...
func TestJoinTopic(t *testing.T) {
	...
	require.NoError(t, tNetworks[1].JoinConsensusTopic())
	require.NoError(t, tNetworks[1].Broadcast(msg, TopicIDConsensus))
}

func TestInvalidTopic(t *testing.T) {
	msg := []byte("test-invalid-topic")

	require.Error(t, tNetworks[1].Broadcast(msg, -1))
}
