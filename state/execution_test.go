...
func TestProposeBlock(t *testing.T) {
	...
	invSubsidyTx := tx.NewMintbaseTx(tState1.lastInfo.BlockHash().Stamp(), 1, tValSigner2.Address(), subsidy, "")
	...
	assert.True(t, b2.Transactions()[0].IsMintbaseTx())
...
func TestExecuteBlock(t *testing.T) {
	...
	t.Run("Subsidy tx is invalid", func(t *testing.T) {
		...
		invBlock := block.MakeBlock(1, util.Now(), txs, tState1.lastInfo.BlockHash(), tState1.stateHash(), tState1.lastInfo.Certificate(), tState1.lastInfo.SortitionSeed(), tState1.signer.Address())
