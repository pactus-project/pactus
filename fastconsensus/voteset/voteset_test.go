...
// This test ensures that `3t` is always less than `2f`.
func TestFaultyPower(t *testing.T) {
	ts := testsuite.NewTestSuite(t)

	powers := []amount.Amount{}
	for i := 0; i < 51; i++ {
		randPower := ts.RandAmount()
		powers = append(powers, randPower)
	}
	valsMap, _, totalPower := setupCommittee(ts, powers...)

	precommitVoteSet := NewPrecommitVoteSet(0, totalPower, valsMap)
	preVoteVoteSet := NewCPPreVoteVoteSet(0, totalPower, valsMap)

	assert.Less(t, 3*precommitVoteSet.thresholdPower(), 2*preVoteVoteSet.faultyPower())
}
