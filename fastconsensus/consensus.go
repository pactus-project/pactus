...
import (
	...
	"github.com/pactus-project/pactus/fastconsensus/log"
...
type consensus struct {
	...
	blockCert       *certificate.BlockCertificate
...
func (cs *consensus) AddVote(v *vote.Vote) {
	...
	if v.Type() == vote.VoteTypeCPPreVote ||
		...
		err := cs.changeProposer.cpCheckJust(v)
		if err != nil {
...
func (cs *consensus) strongCommit() {
	...
	if prepareQH != nil {
		if prepares.HasAbsoluteQuorum(*prepareQH) {
			...
			cs.blockCert = cs.makeBlockCertificate(votes, true)
