...
func TestAllImportedPrivateKeysAddresses(t *testing.T) {
	...
	importedPrvAddrs := td.vault.AllImportedPrivateKeyAddresses()
...
func TestRecover(t *testing.T) {
	...
	t.Run("Invalid mnemonic", func(t *testing.T) {
		...
		_, err = CreateVaultFromMnemonic("invalid mnemonic phrase seed", 21888, dbInstance)
		assert.Error(t, err)
	})
	...
	t.Run("Ok", func(t *testing.T) {
		...
		recovered, err := CreateVaultFromMnemonic(td.mnemonic, 21888, dbInstance)
		assert.NoError(t, err)
...
func TestGetPrivateKeys(t *testing.T) {
	...
	t.Run("No password", func(t *testing.T) {
		addr := td.vault.Addresses()[0].Address
	...
	t.Run("Check all the private keys", func(t *testing.T) {
		for _, info := range td.vault.Addresses() {
			...
			assert.NoError(t, err)
			i := td.vault.Address(info.Address)
...
func TestUpdatePassword(t *testing.T) {
	...
	t.Run("Change password", func(t *testing.T) {
		...
		for _, info := range infos {
			assert.Equal(t, info, td.vault.Address(info.Address))
		}
	})
...
func TestSetLabel(t *testing.T) {
	...
	t.Run("Update label", func(t *testing.T) {
		testAddr := td.vault.Addresses()[0].Address
