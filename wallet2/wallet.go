...
func Create(walletPath, mnemonic, password string, chain genesis.ChainType) (*Wallet, error) {
	...
	vlt, err := vault.CreateVaultFromMnemonic(mnemonic, coinType, database)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) MakeBondTx(sender, receiver, pubKey string, amount int64,
	...
	if pubKey == "" {
		...
		info := w.store.Vault.Address(receiver)
		if info != nil {
...
func (w *Wallet) BroadcastTransaction(trx *tx.Tx) (string, error) {
	...
	err = w.store.History.addTransaction(
		trx.Payload().Signer().String(),
		trx.Payload().Value(),
		id,
		trx.LockTime(),
		trx.LockTime(), // it's wrong ,check it in PR
		trx.Payload().Type().String(),
		d, db.Pending,
	)
	if err != nil {
		return "", err
...
func (w *Wallet) AddTransaction(id tx.ID) error {
	...
	if w.store.Vault.Contains(sender) {
		...
		txID, err := hash.FromBytes(trxRes.Transaction.Id)
		if err != nil {
			return err
		}
		...
		err = w.store.History.addTransaction(
			...
			trxRes.BlockHeight,
			trxRes.BlockTime,
			payload.Type(trxRes.Transaction.PayloadType).String(),
			trxRes.Transaction.Data,
			db.Confirmed,
		)
		if err != nil {
			return err
...
func (w *Wallet) GetHistory(addr string) ([]db.Transaction, error) {
