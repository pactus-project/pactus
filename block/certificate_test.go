...
func TestCertificateersHash(t *testing.T) {
	...
	cert1 := NewCertificate(temp.BlockHash(), temp.Round(), []int{0, 1, 2, 3}, []int{}, temp.Signature())
	...
	cert2 := NewCertificate(temp.BlockHash(), temp.Round(), []int{0, 1, 2, 3}, []int{2, 3}, temp.Signature())
	assert.Equal(t, cert2.CommitteeHash(), cert1.CommitteeHash())
	assert.Equal(t, cert2.Committers(), []int{0, 1, 2, 3})
	assert.Equal(t, cert2.Absences(), []int{2, 3})
	assert.NoError(t, cert2.SanityCheck())

	cert3 := NewCertificate(temp.BlockHash(), temp.Round(), []int{0, 1, 2, 3}, []int{1}, temp.Signature())
	assert.Equal(t, cert3.CommitteeHash(), cert1.CommitteeHash())
	assert.Equal(t, cert3.Committers(), []int{0, 1, 2, 3})
	assert.Equal(t, cert3.Absences(), []int{1})
