...
func TestEvaluation(t *testing.T) {
	...
	t.Run("Total stake is zero", func(t *testing.T) {
		...
		ok, proof := sortition.EvaluateSortition(seed, signer, 0, threshold)
		require.True(t, ok)
		ok = sortition.VerifyProof(seed, proof, signer.PublicKey(), 0, threshold)
		require.True(t, ok)
	})
	...
	t.Run("Total stake is not zero, but validator stake is zero", func(t *testing.T) {
		...
		ok, _ := sortition.EvaluateSortition(seed, signer, total, 0)
		require.False(t, ok)
	})
	...
	t.Run("OK!", func(t *testing.T) {
		...
		require.False(t, sortition.VerifyProof(ts.RandomSeed(), proof1, signer.PublicKey(), total, total/10))
	})
}
