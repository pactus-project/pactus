...
func TestAccountChange(t *testing.T) {
	...
	t.Run("Retrieve an account from store and update it", func(t *testing.T) {
		...
		bal := acc.Balance()
		td.fakeStore.FakeAccounts[addr] = acc
...
func TestValidatorChange(t *testing.T) {
	...
	t.Run("Retrieve an validator from store and update it", func(t *testing.T) {
		...
		addr := val.Address()
		stk := val.Stake()
		td.fakeStore.FakeValidators[addr] = val
...
func TestCreateDuplicated(t *testing.T) {
	...
	t.Run("Try creating duplicated account, Should panic", func(t *testing.T) {
		assert.Panics(t, func() {
			addr, acc := td.GenerateTestAccount()
			td.fakeStore.UpdateAccount(addr, acc)
	...
	t.Run("Try creating duplicated validator, Should panic", func(t *testing.T) {
		assert.Panics(t, func() {
			pub, _ := td.RandBLSKeyPair()
			td.fakeStore.FakeValidators[pub.ValidatorAddress()] = td.GenerateTestValidator()
...
func TestValidatorDeepCopy(t *testing.T) {
	...
	t.Run("existing validator", func(t *testing.T) {
		...
		addr := val1.Address()
		td.fakeStore.FakeValidators[addr] = val1
...
func TestVerifyProof(t *testing.T) {
	...
	for {
		...
		val = td.GenerateTestValidator(
			...
			testsuite.ValidatorWithStake(amount.Amount(td.sbx.totalPower)),
		)
	...
	t.Run("invalid proof", func(t *testing.T) {
		...
		assert.False(t, td.sbx.VerifyProof(lockTime, invalidProof, val))
	})

	t.Run("Ok", func(t *testing.T) {
		assert.True(t, td.sbx.VerifyProof(lockTime, proof, val))
	})
}
...
func TestJoinedToCommittee(t *testing.T) {
	...
	val := td.GenerateTestValidator()
	addr := val.Address()
	td.fakeStore.UpdateValidator(val)
	_ = td.sbx.Validator(addr)
