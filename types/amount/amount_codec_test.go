...
func TestAmount_JSONMarshaling(t *testing.T) {
	...
	t.Run("unmarshal non-string JSON value returns error", func(t *testing.T) {
		var amt amount.Amount
		err := json.Unmarshal([]byte("1234"), &amt)
		assert.Error(t, err)
	})
}
...
func TestAmount_YAMLMarshaling(t *testing.T) {
	...
	t.Run("unmarshal invalid YAML returns error", func(t *testing.T) {
		var amt amount.Amount
		err := yaml.Unmarshal([]byte(`"invalid"`), &amt)
		assert.Error(t, err)
	})

	t.Run("unmarshal unsupported YAML value returns error", func(t *testing.T) {
		var amt amount.Amount
