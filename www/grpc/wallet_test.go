...
func TestListAddress(t *testing.T) {
	...
	t.Run("List addresses successfully", func(t *testing.T) {
			...
			Return([]*types.AddressInfo{
				{
					...
					PublicKey: "pub1",
					Path:      "path1",
					Type:      crypto.AddressTypeEd25519Account,
				},
				{
					Address:   "addr2",
					Label:     "label2",
					PublicKey: "pub2",
					Path:      "path2",
					Type:      crypto.AddressTypeValidator,
				},
			}, nil)
		...
		assert.Equal(t, "addr2", res.Addrs[1].Address)
...
func TestUnencryptedWallet(t *testing.T) {
	conf := testConfig()
	conf.EnableWallet = true

	td := setup(t, conf)

	// Restart the server to test unencrypted wallet.
	td.FakeWalletMgr.EXPECT().ListWallets().Return([]string{"unencrypted_wallet"}, nil).Times(1)
	td.FakeWalletMgr.EXPECT().WalletInfo("unencrypted_wallet").Return(&types.WalletInfo{
		Encrypted: false,
	}, nil).Times(1)

	err := td.Server.StartListening(td.listener)
	require.ErrorContains(t, err, "wallet is not encrypted: unencrypted_wallet")
}
