...
func TestGetTransaction(t *testing.T) {
	...
	t.Run("Should return transaction", func(t *testing.T) {
		...
		assert.Equal(t, tx1.ID().RawBytes(), res.Tranaction.Id)
		assert.Equal(t, tx1.Stamp().RawBytes(), res.Tranaction.Stamp)
		assert.Equal(t, tx1.Fee(), res.Tranaction.Fee)
		assert.Equal(t, tx1.Memo(), res.Tranaction.Memo)
		assert.Equal(t, int32(tx1.Sequence()), res.Tranaction.Sequence)
		assert.Equal(t, tx1.Signature().RawBytes(), res.Tranaction.Signature)
